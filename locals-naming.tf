locals {
  ...
  hostname              = coalesce(var.computer_name, local.name)
  os_disk_name          = coalesce(var.os_disk_custom_name, "${local.name}-osdisk")
  public_ip_name        = coalesce(var.public_ip_custom_name, data.azurecaf_name.pub_ip.result)
  ...
  dcr_name              = coalesce(var.dcr_custom_name, format("dcra-%s", azurerm_linux_virtual_machine.main.name))
}
