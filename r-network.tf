locals {
  public_ip_grp = concat(azurerm_public_ip.main[*], data.azurerm_public_ip.public_ip[*])
}


data "azurerm_public_ip" "public_ip" {
  count = var.public_ip_id == null ? 0 : 1
...
resource "azurerm_network_interface" "main" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = one(local.public_ip_grp[*].id)
  }
